...
const companyService = serviceConfig.baseUrl
    ? new CompanyServiceRest(serviceConfig.baseUrl, dataChangeFn)
    : new CompanyService();
...
const employeeTable = new DataGrid(
    'employees-table-place',
    employeeColumns,
    { title: 'List of Employees', width: '80vw' },
    [
        { name: 'remove', callbackFn: removeEmployee },
        { name: 'update', callbackFn: updateEmployee },
    ],
);
...
function dataChangeFn(employees) {
    switch (menu.getActiveIndex()) {
        case 0:
            ...
            console.log('Update Table of Employees');
            break;
        case 2:
            statisticsProcessing(employees);
...
async function statisticsProcessing(employees) {
    const ageStatisticsData = await action(
        companyService.getStatistics.bind(companyService, age.field, age.interval, employees),
    );
    ageStatistics.fillData(ageStatisticsData);

    const salaryStatisticsData = await action(
        companyService.getStatistics.bind(companyService, salary.field, salary.interval, employees),
    );
...
async function createRandomEmployees() {
    ...
    if (employees.length < 5) {
        for (let i = 0; i < N_EMPLOYEES; i++) {
            await companyService.addEmployee(
